function [u_opt,problem_status] = qp_DeeP_LCC(Up,Yp,Uf,Yf,Ep,Ef,...
    uini,yini,eini,weight_v, weight_s, weight_u,r,lambda_g,lambda_y,u_limit,s_limit)
% =========================================================================
...
Q_blk    = zeros(p*N);
R_blk    = zeros(m*N); 
Q_v         = weight_v*eye(p-m);  % penalty for velocity error
Q_s         = weight_s*eye(m);    % penalty for spacing error
Q           = blkdiag(Q_v,Q_s); 
R = weight_u*eye(m);
for i = 1:N
...
B       = [Up;Ep;Ef];
% c       = [uini_col;eini_col;-5*ones(N,1)];
...
options = optimoptions('quadprog','MaxIterations',1e3,'Display','off');
% Optimization
