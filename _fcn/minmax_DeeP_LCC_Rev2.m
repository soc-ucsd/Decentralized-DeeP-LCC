...
function [uout, problem_status] = minmax_DeeP_LCC_Rev2(UP,YP,UF,YF,WP,WF,...
...
%     K = ones(m2*N, 1);
    ...
    Wc = [ml <= w1 <= mp, uncertain(w1), cone([t1;chol(nearestSPD(A))*[xini1';u1;w1;tauy1]]), yl <= Sf_blk*YF*invH*[xini1';u1;K*w1;tauy1] <= ym];
    
    objective1 = t1;
    
    controller2  = optimizer([Uc,Wc],objective1,[],{xini1},u1);
    [uout, problem_status] = controller2(Xini);
else
    [uout, problem_status] = controller2(Xini);
end
