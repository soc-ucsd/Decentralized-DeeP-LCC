...
%Constant Constraints: [u_min, u_max] vectors, [s_min, s_max] scalar, 
%[d_min, d_max] vectors
function [uout, problem_status, time_comp] = DeeP_LCC_Const_Dual_Test(UP,YP,UF,YF,WP,WF,...
    uini,yini,eini,weight_v,weight_s,weight_u,lambda_g,lambda_y,u_limit,s_limit)
persistent m1 m2 p Tini dim_ini dim_u1 dim_safe dim_t1 dim_tauy1 dim_dis ...
           dim_lambda_i dim_lambda M_obj_c Kone b A c dim_temp M_safe 
if isempty(m1)
    %Get dimension for control system from input data
    ...
    dim_cone = Kone.f + Kone.l + sum(Kone.q);
    ...
    A = zeros(dim_cone, (dim_t1 + dim_u1 + dim_tauy1 + 2 * dim_lambda));
    c = zeros(dim_cone, 1);
    ...
    for i = 1:dim_safe
        ...
        A(((i-1)*2*dim_dis+1):(i*2*dim_dis-dim_dis), :) = Aep'*M_temp1;
        c(((i-1)*2*dim_dis+1):(i*2*dim_dis-dim_dis), 1) = m_ep';
        ...
        A(((i-1)*2*dim_dis+dim_dis+1):(i*2*dim_dis), :) = Aep'*M_temp2;
        c(((i-1)*2*dim_dis+dim_dis+1):(i*2*dim_dis), :) = -m_ep';
    end
    ...
    A(2*dim_safe*dim_dis+2*dim_safe+1:2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda, :) = -M_temp3;
    c(2*dim_safe*dim_dis+2*dim_safe+1:2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda, :) = zeros(2*dim_lambda, 1);
    ...
    A(2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+1:2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1, :) = [Asel; -Asel];
    dim_temp = size(M_temp4, 1);
    for i = 1:2^dim_dis
        A(2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1+(i-1)*dim_temp+1:...
          2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1+i*dim_temp, :) = -M_temp4;
    end
end
...
%If no velocity difference, assume the perturbation is 0.1
if dis_min == dis_max
    dis_min = dis_min - 0.1;
end
...
for i = 1:dim_safe
    ...
    A(2*dim_safe*dim_dis+2*i-1, :) = M_temp1;
    c(2*dim_safe*dim_dis+2*i-1, 1) = s_max-m_c*xini';
    ...
    A(2*dim_safe*dim_dis+2*i, :) = M_temp2;
    c(2*dim_safe*dim_dis+2*i, :) = -s_min+m_c*xini';
end
...
%Represent the input limitation constraint
%Get u from implicit x
c(2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+1:2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1, :) = [u_max; -u_min];


for i = 1:2^dim_dis
    w1 = vertices(:, i);
    vc = [xini'; w1]; %Constant
    c(2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1+(i-1)*dim_temp+1:...
          2*dim_safe*dim_dis+2*dim_safe+2*dim_lambda+2*dim_u1+i*dim_temp, 1) = [zeros(dim_t1); M_obj_c*vc];
end
