...
%Load data sets
load("_data\simulation_data\HDV\Brake_HDV.mat")
S_HDV = S;
...
color_light_orange = "#ffd6a5";
color_green = [119, 172, 48]/255;
color_light_green = '#b7e4c7';
color_purple = '#8661c1';
color_light_purple = '#efbcd5';
label_size  = 18;
total_size  = 14;
line_width  = 2;
...
for i = 1:n_vehicle
    if ID(i) == 1
        plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i,2),'Color',color_gray,'linewidth',line_width-0.5); hold on; % line for velocity of HDVs
        plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i+1,2),'Color',color_purple,'linewidth',line_width); hold on; % line for velocity of CAVs
%         plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i+2,2),'Color',color_gray,'linewidth',line_width-0.5); hold on; % line for velocity of HDVs
%         plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i+4,2),'Color',color_gray,'linewidth',line_width-0.5); hold on; % line for velocity of HDVs
    end 
end
S = S_zero;
for i = 1:n_vehicle
...
hLegend = legend({'NA','NA','HDV','NA','$\mathtt{DeeP-LCC}$','NA','R-$\mathtt{DeeP-LCC}$'}, 'Interpreter','latex');
hLegend.AutoUpdate = 'off';
hLegend.PlotChildren = hLegend.PlotChildren([3,5,7]);
...
for i = 1:n_vehicle
   if ID(i) == 1
       plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i,1)-S(begin_time/Tstep:end_time/Tstep,i+1,1),'Color',color_purple,'linewidth',line_width); hold on; % line for velocity of CAVs
   end 
end
S = S_zero;
for i = 1:n_vehicle
...
annotation(fig,'textbox',...
    [0.59,0.25,0.33,0.09],'Color',[1 0 0],...
    'String','Safe Bound',...
    'FontSize',13,...
...
legend({'HDV','$\mathtt{DeeP-LCC}$', 'R-$\mathtt{DeeP-LCC}$'}, 'Interpreter','latex',...
          'EdgeColor','none');
